...
relayName = ["RLY1","RLY2","RLY3","RLY4","RLY5","RLY6","RLY7","RLY8"]
...
#    print("On Connect")
    ...
    relayStatus=userData.do_status("status")
    base = '/home/outside/'

    count=0
    for v in relayStatus:
        topic = base + relayName[count] + "/state"
...
#    print("On Message ...")
    ...
    idx=relayName.index(tmp[3])
    relayNumber=idx+1

    cmd=""

    if state == 'ON':
        cmd = "close:" + str(relayNumber)
        userData.do_close(cmd)
    else:
        cmd = "open:" + str(relayNumber)
        userData.do_open(cmd)
...
def main(argv=sys.argv):
    ...
    try:
        opts,args = getopt.getopt(sys.argv[1:], "r:vc:ha:", ["relay=","verbose","config=","help","address="])
        for o,a in opts:
            if o in ["-h","--help"]:
                usage()
                sys.exit()
    ...
    if len(relayAddress) > i and traceFlag:
        trace = True
        i += 1

    sr_201 = Sr201(relayAddress)
    i += 1
    ...
    mqttClient = mqtt.Client(userdata=sr_201)
